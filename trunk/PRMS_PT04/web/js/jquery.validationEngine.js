	...
	var methods = {
		...
		_validateField : function(field, options, skipAjaxValidation) {
			...
			for ( var i = 0; i < rules.length; i++) {
				...
				}else if(rules[i] == "custom" ){
					   if(fieldValue.length>0){
						errorMsg = methods._getErrorMessage(form, field, rules[i],
								rules, i, options, methods._custom);
					   }
				}else if(rules[i] == "groupRequired" ){
				...
				}else if(rules[i] == "minSize" ){
					if(fieldValue.length>0){
						errorMsg = methods._getErrorMessage(form, field, rules[i],
								rules, i, options, methods._minSize);
					}
					
				}else if(rules[i] == "maxSize" ){
					if(fieldValue.length>0){
						errorMsg = methods._getErrorMessage(form, field, rules[i],
				...
				}else if(rules[i] == "maxCheckbox" ){
					errorMsg = methods._getErrorMessage(form, field, rules[i],
							rules, i, options, methods._maxCheckbox);
					field = $(form.find("input[name='" + fieldName + "']"));
				
				}else if(rules[i] == "minCheckbox" ){					
					errorMsg = methods._getErrorMessage(form, field, rules[i],
			...
			// validated
			if (!required && field.val() == "" && !errorMsg)
				options.isError = false;
		...
		_getErrorMessage : function(form, field, rule, rules, i, options,
			...
			var custom_message = methods._getCustomErrorMessage(id,
					element_classes_array, rule, options);
		...
		_dateCompare : function(start, end) {
			...
			//check if date1's year is greater then date2's year; false
			if (dateParts1[2] > dateParts2[2]){
				return false;
			}else{
				//checks if date1's month is greater than date2's month; false
				if (dateParts1[1] > dateParts2[1]){
					return false;
				}
				//checks if date1's month is equal to date2's month; have to check the day
				else if (dateParts1[1] == dateParts2[1]){
					//checks if date1's day is greater than date2's day; false
